...
async function refreshWithinBudget(s: Sql, budget: RequestDeadline, signal: AbortSignal): Promise<ShuyuanStats> {
  ...
  // 🔴 host 门必须在 readMeta 解析快照（:378 的 canProbe）与构建探测队列（:786）**之前**刷新——
  // 与 getReadingPool:185-189「刷门要排在 readMeta 之前」是同一个坑的另一处实例。冷启动的 cron
  ...
  // 开关关时保持与今天逐字节相同（不查准入表、不加 DB 往返），与 getReadingPool:189 一致。
