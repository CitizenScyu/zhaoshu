...
#   当口吻词裸放会把叙述拉进删除（`今天，村里人聚在祠堂求月票。`/`本章讲述主角如何求推荐票。`/
#   `上架之后，读者纷纷求订阅。`/`首订那天，书生在街上求打赏。`/`今天求月票的人特别多。` 全被误删）。
#   故把这 6 个词从 `_PLEA_AUTHOR_RE` 移出，改为 `_PLEA_META_PLEA_RE`：**必须与「求」邻接**——
#   元词 +（可选 `第N天/第N日`，或 `更新/加更`）+（可选 `了/的/啦`）+（可选一个分句标点）+ 紧接求告词，
#   且求票词后是分句尾，才判作者求票。于是 `今天求月票！`/`上架了，求订阅！`/`上架第一天，求订阅！`
#   /`今天更新，求订阅！` 仍删，而 `今天求月票的人特别多。`（`求月票` 后接「的…」）、
#   `上架之后，读者纷纷求订阅。`（元词与「求」之间有「之后读者」）保留。
#   注意 `更新/加更` 只作连接槽、仍在**元词之后**：`本章加更，主角去求月票。` 的「求」前是「主角去」
#   而非元词，故仍保留（裸的 `N更/加更` 口吻词在 `_PLEA_AUTHOR_RE` 里照旧生效）。
...
_PLEA_AUTHOR_RE = re.compile(
    ...
    + r'|投我|投给我|给我投|[一二三四五六七八九十两0-9]更|加更')
_PLEA_CLAUSE_TAIL = r'(?:啦|了|呀|吧|哦|喔|啊)?(?:[！!。．…~～、，,；;\s]|$)'
_PLEA_META_PLEA_RE = re.compile(
    r'(?:今天|今日|明天|本章|上架|首订)(?:第[0-9一二三四五六七八九十两]+[天日]|更新|加更)?'
    r'(?:了|的|啦)?[，,、。]?\s*'
    r'(?:拜求|跪求|再求|还求|求)(?:一下|一波|下|个|张)?'
    r'(?:推荐票|推荐|收藏|月票|订阅|打赏|票+)' + _PLEA_CLAUSE_TAIL)
...
def _drop_rule(line: str) -> str | None:
    ...
    #    或行内有求票词 / 单用「求票」分句且带作者口吻信号；或更新元词（今天/本章/上架/首订…）
    #    与「求票词」紧邻成句。叙述行不删（lblqualfix41/lblfu41/lblfurev41）。
