...
//
// 取值 = 120s（2026-09-17 定，取代此前的 5_000）。依据是兜底模型 claude-opus-5-88 的
// **实测整通耗时**，不是它的首字节：
//   - llm_usage row 214 一次真实兜底：正文 3458 token 还流了约 76s，**整通 89.6s**；
//   - 中位 **115.8s**（见本文件上方「找书全链路换用快模型」那段注释）。
// ⚠️ 别拿 ttfbMs（13.6s）当量级——那是首字节，不是整通；会把这个门槛算小一个数量级。
// 门槛略高于中位 ⇒ 剩余预算跑不完「中位耗时的一次兜底」时不再发起。旧值 5s 挡不住任何东西：
// 主模型一次单次尝试上限就是 45s，剩 6–89s 时它照样开兜底，那次兜底大概率跑不完，只会把
// 剩余预算烧光，并把错误渲染成「LLM 总超时（剩余预算 Xs）」——用户读到「模型太慢」，真实
// 原因是「主模型已降级 + 兜底预算不够」。线上「兜底只剩 29.987s」就是这一格（llm_usage row 214）。
// 剩余不够时**原样抛主模型的错误码**（不新造错误），错误码因此仍指向「主模型卡住」这一层。
// 可用 LLM_TOTAL_TIMEOUT_MS 调总预算；这个门槛本身刻意不做成环境变量——它是从实测耗时
// 推出来的物理量，不是运维旋钮。
export const MODEL_FALLBACK_MIN_BUDGET_MS = 120_000;
...
export async function chatRobust(
  ...
  try {
    ...
    // 剩余预算跑不完「中位耗时的一次兜底」（MODEL_FALLBACK_MIN_BUDGET_MS，取实测 120s）时
    // **不发起**，把主模型的错误原样抛出去：错误码因此仍指向「主模型卡住」这一层，而不是被
    // 一次注定超时的兜底改写成「LLM 总超时」（见该常量的注释）。
