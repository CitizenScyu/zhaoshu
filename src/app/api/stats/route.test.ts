...
describe('GET /api/stats', () => {
  ...
  it('returns aggregate data and explicit availability for each partition', async () => {
    ...
    expect(await res.json()).toEqual({
      ...
      shuyuan: { total: 10, active: 8 }, tokens: null,
      availability: { library: true, download: true, find: true, shelf: true, shuyuan: true },
    });
  ...
  it('keeps real empty aggregates at zero with available=true', async () => {
    ...
    expect(await res.json()).toEqual({
      ...
      shelf: { statuses: [] }, shuyuan: { total: 0, active: 0 }, tokens: null,
      availability: { library: true, download: true, find: true, shelf: true, shuyuan: true },
    });
  });
