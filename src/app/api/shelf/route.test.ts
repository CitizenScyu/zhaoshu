...
function request(method: 'POST' | 'DELETE', body?: unknown, bookId?: string) {
  const url = new URL('http://localhost/api/shelf');
  if (bookId !== undefined) url.searchParams.set('bookId', bookId);
...
describe('/api/shelf', () => {
  ...
  it.each([7, '7', 2147483647])('adds valid book %s as want without changing its identity', async (labeledBookId) => {
    ...
    // F09：展示列存原始拼写（含首尾空白，身份键由生成列归一）。
    expect(db.queries.find((query) => query.text.includes('INSERT INTO books'))?.values).toEqual([' 测试书 ', '佚名']);
    // F08：status 经 COALESCE(最新反馈, 'want')，绑定顺序：userId / '书库添加' / 反馈子查询 userId / 'want' / 归一身份键 / 查重 userId。
    expect(db.queries.find((query) => query.text.includes('INSERT INTO recommendations'))?.values).toEqual([1, '书库添加', 1, 'want', '测试书', '佚名', 1]);
  });
  ...
  it.each([undefined, '', '0', '1.5', 'Infinity', '2147483648', '9007199254740992'])('rejects DELETE bookId %s without writes', async (bookId) => {
    const res = await DELETE(request('DELETE', undefined, bookId));
    expect(res.status).toBe(400);
    expect(await res.json()).toEqual({ error: 'missing valid bookId', code: 'INVALID_ID' });
    expect(ensureSchema).not.toHaveBeenCalled();
    expect(sql).not.toHaveBeenCalled();
  });

  it('deletes a valid book shelf and reports an absent one', async () => {
    sql.mockResolvedValueOnce([{ id: 2147483647 }]);
    const deleted = await DELETE(request('DELETE', undefined, '2147483647'));
    expect(deleted.status).toBe(200);
    // F05：按 (user_id, book_id) 删除，值顺序 [userId, bookId]。
    expect(sql.mock.calls[0][0].values).toEqual([1, 2147483647]);
    expect(sql.mock.calls[0][0].text).toContain('WHERE user_id = ? AND book_id = ?');
  ...
  it('伪造他人的 bookId 与不存在的 bookId 都是 404，且不改变任何状态', async () => {
    // bookId -> 归属 userId
    const owners = new Map([[7, 3], [8, 2]]);
    sql.mockImplementation((query) => {
      if (!query.text.includes('DELETE FROM recommendations')) return [];
      const [userId, bookId] = query.values.map(Number);
      if (owners.get(bookId) !== userId) return [];
      owners.delete(bookId); return [{ id: bookId }];
    });
    ...
    for (const query of db.queries.filter((q) => q.text.includes('DELETE FROM recommendations'))) {
      expect(query.text).toContain('WHERE user_id = ? AND book_id = ?'); expect(query.values[0]).toBe(2);
    }
  });
