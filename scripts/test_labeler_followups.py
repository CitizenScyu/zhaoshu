...
class TestPleaMetaWordNarrowing(unittest.TestCase):
    ...
    裸放会把叙述行拉进删除。收窄为 `_PLEA_META_PLEA_RE`：元词必须与「求…票类词」
    **同属一个分句**（中间只隔连接词/标点/空白），且求票词**收尾**（尾部是分句结束标点或行尾）。
    `，`/`、`/空白 不算收尾——那意味着后面还有续句，而续句大多是叙述。
...
class TestPleaJiagengNarrowing(unittest.TestCase):
    """jiageng41：口吻词「加更」原裸放在 `_PLEA_AUTHOR_RE`，只要行内同时出现「加更」与
    求票词就删，误删 `今天加更，求月票的读者很多。` 一类叙述（穷举里约 900 条）。
    收窄：把「加更」从 `_PLEA_AUTHOR_RE` 移出、并入 `_PLEA_META_PLEA_RE` 的起始锚，
    与 6 个元词同一套「同分句 + 严尾」判据——求票词收尾才删，续句叙述保留。
    注：`二更/三更/四更` 等 `[一二三四…]更` 不在收窄范围，仍走 AUTHOR 分支。"""
    # 含「加更」、无引号、无第三人称，但求票词后仍有续句（未收尾）→ 必须保留
    NARRATION_KEEP = (
        '今天加更，求月票的读者很多。',
        '上架加更，求订阅的读者很多。',
        '加更求订阅的读者很多。',
    ...
    PLEA_DROP = (
        ...
        '加更！求收藏！',
        '加更求月票！',
        # jiagengrev41 必修：「加更 + 交代槽（章/字数/奉上/到/完毕…）+ 求票尾」硬样本
        # （第一轮 9 条 PLEA_DROP 没覆盖这一族，套件全绿掩盖了漏删）
        '今天加更三章，求月票！',
        '加更奉上，求推荐票～',
        '加更到！求收藏。',
        '为盟主加更，求订阅',
    ...
    def test_jiageng_narration_kept(self):
        for line in self.NARRATION_KEEP:
            self.assertIsNone(labeler._drop_rule(line), line)

    def test_jiageng_plea_dropped(self):
        for line in self.PLEA_DROP:
            self.assertEqual(labeler._drop_rule(line), 'plea', line)
