...
- **LLM 召回**：模型训练语料里泡着十几年知乎/龙空/贴吧书评，按画像+需求召回 15~20 本候选，冷门佳作优先于榜单常客。
...
```bash
cp .env.local.example .env.local   # 填好三个变量
npm install
npm run dev
```
...
1. 推到 GitHub 私有仓库
2. [neon.tech](https://neon.tech) 建免费库，拿连接串
3. Vercel 导入仓库，配 Environment Variables（同 `.env.local.example` 三项）
4. （推荐，国内直连）把 `find.cloud.us.kg` 之类子域 DNS-only CNAME 到 `cname.vercel-dns.com`，在 Vercel 项目里 Add Domain

注意：Hobby 档单函数 60s 上限，流水线已拆为 recall / verify / rerank 三步由前端分步调用。
