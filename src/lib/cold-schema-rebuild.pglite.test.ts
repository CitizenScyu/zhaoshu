// 灾备重建链路的端到端真库验收（41-COLD-SCHEMA，dr41 改为全链路；41-coldbuild 补 artifact 段）。
//
// 冷建库的正式顺序是 `db:migrate`（0001→0003）→ `migrate:auth:prod`（initializeAuthSchema 补 auth v5-v7）
// → `migrate:artifacts:prod`（initializeArtifactSchema 补 artifact schema v1）→ 运行时 ensureSchema
// （assertAuthSchema + initializeBusinessSchema）。本文件用 PGlite 真执行这条链，每一步都走仓内真代码
// （迁移 runner、db:check 判定、两个生产迁移入口），不 mock 任何 SQL。
//
...
// 业务 schema 初始化成功为止。同理 41-coldbuild 补上 artifact 段：不跑 migrate:artifacts:prod 时
// T8 worker 启动即 `relation "storage_repositories" does not exist`（tempdb41 §缺陷 D1）。
//
// 不变量：
// 1. 已发布迁移的摘要冻结（PUBLISHED_CHECKSUMS）——改已发布文件的字节会让生产 db:check/db:migrate 拒绝。
// 2. 只跑 db:migrate 时 db:check 判不通过（auth 只记到 v4、artifact 记账表都还没有），且这两项都是原因。
// 3. 跑完 migrate:auth:prod 与 migrate:artifacts:prod 后 db:check 通过，ensureSchema 的两步都成功。
...
maybe('冷建库灾备链路（db:migrate → migrate:auth:prod → ensureSchema，全真 SQL）', () => {
  ...
  it('db:migrate 后全部版本已记账、业务表齐全，但 auth 只到 v4、artifact 未建 → db:check 不通过', async () => {
    ...
    // 冷建库在跑 migrate:artifacts:prod 之前没有 artifact 三表：这里与 auth 一起构成不通过的两类原因。
    expect([...verdict.missingTables].sort()).toEqual(['artifact_schema_migrations', 'book_artifacts', 'storage_repositories']);
    expect(verdict.checksumOk).toBe(true);
    expect(verdict.authVersion).toBe(4);
    expect(verdict.authVersionOk).toBe(false);
    // 冷建库在跑 migrate:artifacts:prod 之前没有 artifact 记账表：这一项也必须判不通过
    // （tempdb41 §缺陷 D1：此前 db:check 照样 rc=0，T8 worker 启动即 relation does not exist）。
    expect(verdict.artifactVersion).toBeNull();
    expect(verdict.artifactVersionOk).toBe(false);
    expect(verdict.ok).toBe(false);
    await expect(assertAuthSchema(sql as never)).rejects.toBeInstanceOf(AuthSchemaRequiredError);
  }, 60_000);

  it('migrate:auth:prod 补齐 v5-v7 后 db:check 仍不通过：artifact 记账还没建', async () => {
    ...
    const verdict = evaluateSchema(await inspectSchema(client), migrations);
    expect(verdict.authVersionOk).toBe(true);
    expect(verdict.artifactVersionOk).toBe(false);
    expect(verdict.ok).toBe(false);
  }, 60_000);
  ...
  it('ensureSchema 两步都成功（assertAuthSchema 放行 + initializeBusinessSchema 不报缺列）', async () => {
    ...
    // artifact schema 到位：T8 worker 的 resolveRepositoryId 直查这两张表（tempdb41 §缺陷 D1 的失败点）。
    for (const table of ['storage_repositories', 'book_artifacts']) {
      expect((await pg.query(`SELECT to_regclass($1) IS NOT NULL AS present`, [table])).rows, table).toEqual([{ present: true }]);
    }
