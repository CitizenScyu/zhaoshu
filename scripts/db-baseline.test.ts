...
import { applyMigration, EXPECTED_TABLES, loadMigrations, PUBLISHED_CHECKSUMS } from './db-migration-lib.mjs';
...
maybe('PGlite 真库', () => {
  ...
  describe('(b) 任一效果不成立 → 拒绝、零写入、退出码 2', () => {
    ...
    it.each([
      ...
      ['已有空的记账表', 'CREATE TABLE schema_migrations (version integer PRIMARY KEY, name text NOT NULL, checksum char(64) NOT NULL, applied_at timestamptz NOT NULL DEFAULT now())',
        /已有 schema_migrations（登记了 0 个版本）/],
