...
def main() -> int:
    ...
    ap.add_argument('--source', choices=('rank', 'douban'), default='rank',
    ...
    for i, b in enumerate(queue, 1):
        ...
        try:
            ...
            b_out = {
                ...
                'status': b.get('status', ''),
                'source': 'book15.net',
                # 豆瓣线选出的书记录来源标记，便于与榜单线的产出区分；
                # 正文仍抓自 book15，source 语义不变。
                'selected_by': 'douban-tag' if args.source == 'douban' and not args.book else 'book15-rank',
                'url': BASE + b['url'],
                'chars': chars,
                'labels': labels,
            }
