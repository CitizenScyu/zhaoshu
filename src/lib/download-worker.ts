  ...
  try {
    ...
    if (outcome.kind === 'incomplete') {
      ...
      const written = await storage.finish(lease, {
        status: 'partial',
        error: cleanPgText(`缺章 ${outcome.chaptersDone}/${outcome.chaptersTotal}：${outcome.reason}`.slice(0, 4000)),
      });
...
export function createEngineAdapter(options: EngineAdapterOptions): SourceAdapter {
  ...
  return {
    ...
    async download(task, context) {
      ...
      const classified = isBudgetExhausted(context.signal.reason) ? 'budget_exhausted' : reason;
